...
const postAbsenKantin = async (req, res) => {
    ...
    try {
        ...
        if (usr.kategori == 'karyawan') {
            rekap_user = await Karyawan.findOne({where:{rfid:request.rfid}});
            ...
            rekap_user = await Outsource.findOne({where:{barcode:request.rfid}});
            if (moment(tgl_now).format("HH:mm:ss") >= '11:00:00' && moment(tgl_now).format("HH:mm:ss") <= '13:30:00') {
                if (hadir.length > 0 && hadir.length < 3) {
                    if (moment(hadir[0].createdAt).format("HH:mm:ss") >= '11:00:00' && moment(hadir[0].createdAt).format("HH:mm:ss") <= '13:30:00') {
                        msg = "Anda Sudah Tapping Absen di Sesi 1, Selamat Makan :)";
                        status = 1;
                    } else {
                        const created = await Absen.create({
                            rfid: request.rfid,
                            nrk: rekap_user.nomor_karyawan,
                            createdAt: moment(tgl_now).format("YYYY-MM-DD HH:mm:ss"),
                            updatedAt: moment(tgl_now).format("YYYY-MM-DD HH:mm:ss"),
                            tanggal : moment(tgl_now).format("YYYY-MM-DD"),
                            waktu : moment(tgl_now).format("HH:mm:ss"),
            ...
            rekap_user = await Internship.findOne({where:{barcode:request.rfid}});
            if (moment(tgl_now).format("HH:mm:ss") >= '11:00:00' && moment(tgl_now).format("HH:mm:ss") <= '13:30:00') {
                if (hadir.length > 0 && hadir.length < 3) {
                    if (moment(hadir[0].createdAt).format("HH:mm:ss") >= '11:00:00' && moment(hadir[0].createdAt).format("HH:mm:ss") <= '13:30:00') {
                        msg = "Anda Sudah Tapping Absen di Sesi 1, Selamat Makan :)";
