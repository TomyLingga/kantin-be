...
const getAllKaryawan = async (req, res) => {
    ...
    try {
        const load = await Outsourcing.findAll();
...
const createRfidCard = async (req, res) => {
    ...
    try {
        const load = await Outsourcing.findOne({where:{barcode:request_body.rfid_code}});
        if (load != null) {
            ...
            const data = {
                ...
                nomor_karyawan:request_body.user_id,
            ...
            const post = await Outsourcing.create(data, { transaction: t })
...
const updateRfidCard = async (req, res) => {
    ...
    try {
        ...
        await Outsourcing.update(data, {where:{id: request_body.id}});
        res.send({"code":200, "msg": "Data berhasil disimpan"});
    } catch (err) {
...
const deleteRfidCard = async (req, res) => {
    ...
    try {
        ...
        await Outsourcing.destroy({where:{id: request_body.id}, transaction: t});
